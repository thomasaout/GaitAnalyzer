...
class KinematicsReconstructor:
    ...
    def animate_kinematics(self):
        ...
        markers = Markers(data=self.experimental_data.markers_sorted_with_virtual[:, :, :10], channels=marker_names)

        # Visualization
        viz = PhaseRerun(self.t)
        viz.add_animated_model(model, self.q.reshape(42, 10), tracked_markers=markers)
...
class ConstrainedKalmanRecons:
    ...
    def set_optimal_initial_states(self, markers):
        ...
        # First phase: root estimation
        nb_root_markers = self.model.nbMarkers(0)  # Get number of markers on root segment
        markers_root = markers_flat.copy()
